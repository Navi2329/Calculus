...
print(
    "DID YOU KNOW , Sir Isaac Newton invented calculus in approximately the same amount of time the average student learns it in university.")
...
table.add_row(["PRODUCT RULE: f(x)*g(x)", "f'(x)*g(x)+g'(x)*f(x)", "#", "DIVISION RULE: f(x)/g(x)",
               "(g(x)*f'(x)-f(x)*g'(x))/((g(x))**2"])
print(table)
print(
    "Note: If we desire to bring back the function from the derivative f'(x) , we perform ʃf'(x)dx ,which is out of the scope of this project.")
...
print("Instructions: ")
print(
    'Please give brackets to every constant term and variable seperately and make sure to use only two terms with an operator with brackets to avoid errors')
...
class DivExpr(Expr):
    ...
    def differentiate(self):
        return (
            DivExpr((SubtractExpr(MulExpr(self.denoExpr, self.numExpr.differentiate()),
                                  (MulExpr((self.numExpr), self.denoExpr.differentiate())))),
                    MulExpr(self.denoExpr, self.denoExpr)))
...
class power1(Expr):
    ...
    def differentiate(self):
        return MulExpr(power1(self.expr, self.power), AddExpr(MulExpr(self.power.differentiate(), ln(self.expr)),
                                                              MulExpr(DivExpr(self.expr.differentiate(), self.expr),
                                                                      self.power)))
...
class asin(Expr):
    ...
    def differentiate(self):
        return MulExpr(self.expr.differentiate(), DivExpr(MulExpr(ConstExpr(1), PowerExpr(0)), sqrt(
            SubtractExpr(MulExpr(ConstExpr(1), PowerExpr(0)), MulExpr(self.expr, self.expr)))))
...
class atan(Expr):
    ...
    def differentiate(self):
        return MulExpr(self.expr.differentiate(), DivExpr(MulExpr(ConstExpr(1), PowerExpr(0)),
                                                          AddExpr(MulExpr(ConstExpr(1), PowerExpr(0)),
                                                                  MulExpr(self.expr, self.expr))))
...
class asec(Expr):
    ...
    def differentiate(self):
        return MulExpr(self.expr.differentiate(), DivExpr(MulExpr(ConstExpr(1), PowerExpr(0)), MulExpr(mod(self.expr),
                                                                                                       sqrt(
                                                                                                           SubtractExpr(
                                                                                                               MulExpr(
                                                                                                                   ConstExpr(
            ...
            if ch == 15:
                ...
                if defined() == False:
                    ...
                    if ch == 7:
                        ...
                        if limit(str(func), x, q, '+') == limit(str(func), x, q, '-') and limit(str(func), x,
                                                                                                q) is not zoo:
                    ...
                    if ch == 7:
                        ...
                        if ch == 3:
                            ...
                            if ch == 8:
                                ...
                                print(eq)
                                if str(eq)[0] == 'x':
                                    z1 = plot_implicit(Eq(x, float(str(eq)[2::])), show=False, line_color='red',
                                                       ylim=(-10, 10), legend=True)
                                else:
